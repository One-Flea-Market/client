...
const EmailAuth = () => {
  ...
  return (
    <div className=" flex [&>*]:rounded-xl mb-3 -mt-5 md:-mt-3 -ml-5 md:ml-0 [&>*]:outline-none">
      <input
        type="text"
        className="md:text-lg  text-sm w-[82%] md:w-full my-[0.55rem] h-10 p-2 border-4 border-blue-300"
        ref={ref}
        placeholder="인증번호"
      />
      <input
        ...
        onClick={() => {
          if (ref?.current?.value === auth) {
            setMessage(state => ({
              ...state,
              email: { ...state.email, message: "인증이 완료 되었습니다." }
            }))
            setAuth("")
            ...
            setMessage(state => ({
              ...state,
              email: { ...state.email, message: "인증번호를 다시입력해주세요." }
            }))
        }}
      />
    </div>
